...
// FIXME: This needs a better solution but is a workaround for late night coding
// and not wanting a dependency on scalaz
private final case class ExtensionCode(a: Int, b: Int, c: Int) extends Status {
  override val toInt = "%s%s%s".format(a,b,c).toInt
...
object Status {
   ...
   */
   // not sure what the hell this does (t.p)
  // def status(d1: Digit, d2: Digit, d3: Digit): Status =
  //   "%s%s%s".format(d1,d2,d3).toInt
  ...
  implicit def IntStatus(n: Int): Option[Status] = n match {
    ...
    case _ => 
      if(n >= 0 && n <= 999) Some(ExtensionCode(n / 100, n % 100 / 10, n % 10))
      else None
  }
}