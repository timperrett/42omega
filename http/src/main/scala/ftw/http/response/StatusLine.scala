...
import ftw.http._, request.Request, Version.version
...
object StatusLine {
   ...
   */
  def statusLine(v: Version, s: Status, r: String) = new StatusLine {
    val version = v
    val status = s
    val reasonPhrase = r
  }
   ...
   */
  def statusLine(version: Version, status: Status): StatusLine = 
    statusLine(version, status, status.reasonPhrase.getOrElse(""))
   ...
   */
  def statusLine(status: Status)(implicit req: Request): StatusLine = 
    statusLine(req.line.version, status)
  ...
  implicit def statusLine10(status: Status): StatusLine = 
    statusLine(version(_1, _0), status)
   ...
   * <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec6.html#sec6.1.1">RFC 2616 Section 6.1.1 Status Code and Reason Phrase</a>.
   */
  implicit def statusLine11(status: Status): StatusLine = 
    statusLine(version(_1, _1), status)
}
