...
sealed trait Method {
   ...
   */
  lazy val asList: List[Char] = asString.toList
   ...
   * value.
   */
  def extension[X](f: List[Char] => X, x: => X) = this match {
...
private final case class ExtensionMethod(m: List[Char]) extends Method {
...
object Method extends Methods {
   ...
   */
  def unapply(m: Method) = Some(m.asList)
}